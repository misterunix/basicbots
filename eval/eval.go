...
func New(stream *tokenizer.Tokenizer) (*Interpreter, error) {
	...
	t.RegisterBuiltin("ATN2", 2, builtin.ATN2)
...
func (e *Interpreter) callUserFunction(name string, args []object.Object) object.Object {
	...
	//
	if e.trace {
		fmt.Fprintf(os.Stderr, "Calling user-defined function %s\n", name)
	}
	...
	out := eval.expr(true)
	if e.trace {
		fmt.Fprintf(os.Stderr, "\tCalled expr() - result is\n\t%s\n", out.String())
	}
...
func (e *Interpreter) callBuiltin(name string) object.Object {
	...
	if e.trace {
		fmt.Fprintf(os.Stderr, "callBultin(%s)\n", name)
	}
	...
	if e.trace {
		fmt.Fprintf(os.Stderr, "\tReturn value %s\n", out.String())
	}
	return out
}
...
func (e *Interpreter) RunOnce() error {
	...
	if e.trace {
		fmt.Fprintf(os.Stderr, "RunOnce( %s )\n", tok.String())
	}
...
// RunStep launches the program, and does not return until it is over.
//
// A program will terminate when the control reaches the end of the
// final-line, or when the "END" token is encountered.
// Modified 2021 by Bill Jones for use in basicbots.
func (e *Interpreter) RunStep() error {

	//
	// We walk our series of tokens.
	//
	if e.offset < len(e.program) && !e.finished {
		err := e.RunOnce()
		if err != nil {
			return fmt.Errorf("line %s : %s", e.lineno, err.Error())
		}
